    ...
    def __iter__(self):
        while True:
            ...
            for _ in range(self.song_batch_size):
                ...
                # encode both
                audio_latent = (
                    self.encdec.encode(audio, max_waveform_length=44100 * 30)
                    .squeeze()
                    .to(self.device)
                )
                audio_gain_latent = (
                    self.encdec.encode(audio_gain, max_waveform_length=44100 * 30)
                    .squeeze()
                    .to(self.device)
                )

                # for testing
                # audio_latent = torch.rand_like(audio_latent)
                # audio_gain_latent = gain * audio_latent

                # audio_latent = torch.full([64,47], 1.0).to(self.device)
                # audio_gain_latent = torch.full([64,47], 2.0).to(self.device)
                # audio_gain_latent = gain * audio_latent

                gain_tensor = torch.full((1, audio_latent.shape[-1]), gain).to(
                    self.device
                )
                # gain_tensor = torch.full((1,audio_latent.shape[-1]), 1.0).to(self.device)

                audio_latent = torch.cat(
                    [audio_latent, gain_tensor], 0
                )  # add gain to the end
